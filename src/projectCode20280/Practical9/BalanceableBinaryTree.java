...
import projectCode20280.Practical5.LinkedBinaryTree;
...
public class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
	...
	public void rotate(Position<Entry<K, V>> p) {
		...
		Node<Entry<K, V>> parentPosition = position.getParent();
		Node<Entry<K, V>> grandparentPosition = parentPosition.getParent();
		...
		if(grandparentPosition == null){
			...
			relink(grandparentPosition, position, parentPosition == grandparentPosition.getLeft());
		}

		if(position == parentPosition.getLeft()){
			relink(parentPosition, position.getRight(), true);
			relink(position, parentPosition, false);
		}

		else{
			relink(parentPosition, position.getLeft(), false);
			relink(position, parentPosition, true);
		}
